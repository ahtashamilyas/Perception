...
class YcbVideoReader(BopBaseReader):
  def __init__(self, base_dir, zfar=np.inf):
    ...
    if 'BOP' not in self.base_dir:
      # Try to load keyframe.txt first, fallback to test_targets_bop19.json
      keyframe_file = f'{self.base_dir}/../../keyframe.txt'
      test_targets_file = f'{self.base_dir}/../../test_targets_bop19.json'
      
      if os.path.exists(keyframe_file):
        with open(keyframe_file,'r') as ff:
          self.keyframe_lines = ff.read().splitlines()
        print(f"Loaded {len(self.keyframe_lines)} keyframe entries from keyframe.txt")
      elif os.path.exists(test_targets_file):
        # Generate keyframe lines from test_targets_bop19.json
        with open(test_targets_file,'r') as ff:
          test_targets = json.load(ff)
        ...
        for entry in test_targets:
          ...
          keyframe_set.add(key)
        
        self.keyframe_lines = list(keyframe_set)
        print(f"Generated {len(self.keyframe_lines)} keyframe entries from test_targets_bop19.json")
      else:
        logging.warning("Neither keyframe.txt nor test_targets_bop19.json found, no keyframes will be detected")
        self.keyframe_lines = []
  ...
  def is_keyframe(self, i):
    ...
    # Try both formats: SSSS/FFFFFF and SSSSSS/FFFFFF
    key_4digit = f'{video_id:04d}/{frame_id:06d}'
    key_6digit = f'{video_id:06d}/{frame_id:06d}'
    
    is_keyframe = (key_4digit in self.keyframe_lines) or (key_6digit in self.keyframe_lines)
    
    # Debug print for first few frames
    if i < 5:
      print(f"Debug: Frame {i}, video_id={video_id}, frame_id={frame_id}")
      print(f"  Checking keys: '{key_4digit}' and '{key_6digit}'")
      print(f"  Found in keyframes: {is_keyframe}")
      if len(self.keyframe_lines) > 0:
        print(f"  First few keyframe entries: {self.keyframe_lines[:3]}")
