...
def run_pose_estimation_worker(mask, est:FoundationPose, internal_cam_parameters, ob_id=None, device:int=0, depth_image=None, rgb_image=None):
    ...
    #TODO: service(Obj_name, Pose)
    # Obj1(Red_Cube), #Obj2(Blue_Cube), #Obj3(Yellow_Cube)
    #Service call: service(Obj_name, Pose) -> Time Stamp ->Initialize Pipeline-> Pose


        # with open(f'{opt.debug_dir}/ycbv_res.yml','w') as ff:
        # yaml.safe_dump(make_yaml_dumpable(pose), ff)
