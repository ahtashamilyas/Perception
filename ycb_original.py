...
def run_pose_estimation_worker(reader, i_frames, est:FoundationPose, debug=False, ob_id=None, device:int=0):
  ...
  for i in range(len(i_frames)):
    ...
    #est.gt_pose = reader.get_gt_pose(i_frame, ob_id) #TODO not needed
    print("min, max depth", depth.min(), depth.max())
    print('depth.sum() 2', depth.sum())
    print("min, max color", color.min(), color.max())
    print("color.sum() 2", color.sum())
    print("Mask sum", ob_mask.sum())
    print("color: {}, depth: {}, mask: {} and K: {}".format(color, depth, ob_mask, reader.K))
    print("color dtype: {}, depth dtype: {}, mask dtype: {} and K dtype: {}".format(color.dtype, depth.dtype, ob_mask.dtype, reader.K.dtype))
